...
object RandomDiffToHTMLExport extends App {
  ...
  def randomExport() = {
    ...
    (0 until numDsToExport).foreach(_ => {
      ...
      val dsBeforeChange = IOService.loadSimplifiedRelationalDataset(data.DatasetInstance(lineage.id,firstVersion))
      val dsAfterChange = IOService.loadSimplifiedRelationalDataset(data.DatasetInstance(lineage.id,secondVersion))
