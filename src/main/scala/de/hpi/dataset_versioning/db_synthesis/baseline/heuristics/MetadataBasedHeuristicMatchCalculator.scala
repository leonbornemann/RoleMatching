...
class MetadataBasedHeuristicMatchCalculator extends HeursticMatchCalculator{
  ...
  def calculateMatch(tableA: SynthesizedTemporalDatabaseTableSketch, tableB: SynthesizedTemporalDatabaseTableSketch) = {
    ...
    var toReturn = new TableUnionMatch(tableA,tableB,None,0,true)
    if(!nonKeyAttrA.nameSet.intersect(nonKeyAttrB.nameSet).isEmpty ){
      ...
      if(!valueActiveTimeOverlap.isEmpty){
        ...
        if(!keyActiveTimesA.intersect(keyActiveTimesB).intersect(valueActiveTimeOverlap).isEmpty){
          toReturn = new TableUnionMatch(tableA,tableB,None,Integer.MAX_VALUE,true) //TODO: we could plug in the smallest number of rows here if we store that as metadata
        }
      }
    }
    toReturn
