...
import de.hpi.role_matching.blocking.cbrb.role_tree.AbstractAsynchronousRoleTree.maxPairwiseListSizeForSingleThread
import de.hpi.role_matching.blocking.cbrb.role_tree._
import de.hpi.role_matching.blocking.cbrb.role_tree.bipartite.AsynchronousBipartiteRoleTree
...
class AsynchronousRoleTree(tuples: IndexedSeq[RoleReference],
                           ...
                           var matchingTaskList:NormalPairwiseTaskList = NormalPairwiseTaskList()
        ...
        if(squareProductTooBig(g.nonWildcardRoles.size)){
          ...
          if(g.nonWildcardRoles.size>thresholdForFork){
            ...
            new AsynchronousRoleTree(g.nonWildcardRoles.toIndexedSeq,
              ...
              serializeGroupsOnly,
              matchingTaskList)
            matchingTaskList = NormalPairwiseTaskList()
  ...
  override def getGraphConfig: CBRBConfig = graphConfig
...
object AsynchronousRoleTree extends StrictLogging {
  ...
  def createAsFuture(futures:java.util.concurrent.ConcurrentHashMap[String,Future[Any]],
    ...
    ConcurrentRoleTreeTraverser.setupFuture(f,fname,futures,context)
    f
  }
