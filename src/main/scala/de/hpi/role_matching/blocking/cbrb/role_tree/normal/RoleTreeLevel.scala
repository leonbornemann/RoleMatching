...
import de.hpi.role_matching.blocking.cbrb.role_tree.{IterableRoleIndex, normal}
...
class RoleTreeLevel(private var tuples: IndexedSeq[RoleReference],
  ...
  class TupleGroupIterator(skipWildCardBuckets: Boolean) extends Iterator[RoleTreePartition] {
    ...
    override def next(): RoleTreePartition = {
      ...
      normal.RoleTreePartition(indexedTimestamps,parentNodesKeys ++ Seq(key),tuples,wildcards)
    }
  }

  override def wildcardBuckets: IndexedSeq[RoleTreePartition] = wildcardKeyValues
    .map(k => normal.RoleTreePartition(indexedTimestamps,parentNodesKeys ++ Seq(k),IndexedSeq(),index.getOrElse(k,IndexedSeq())))
    .toIndexedSeq
