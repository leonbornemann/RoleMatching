...
import de.hpi.socrata.metadata.custom.joinability.`export`.Column
import de.hpi.socrata.parser.exceptions.SchemaMismatchException
...
class OldLoadedRelationalDataset(val id:String, val version:LocalDate, val rows:ArrayBuffer[scala.collection.IndexedSeq[JsonElement]]=ArrayBuffer()) extends StrictLogging{
  ...
  def getColumnObject(j: Int) = {
    ...
    Column(id,version.format(GLOBAL_CONFIG.dateTimeFormatter),colNames(j),values)
  }
