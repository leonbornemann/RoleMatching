...
class ChangeExporter extends StrictLogging{
  ...
  def getDatesInRange(startInclusive: LocalDate, endExclusive: LocalDate) =
    (startInclusive.toEpochDay until endExclusive.toEpochDay)
      .map(l => LocalDate.ofEpochDay(l))

  def exportAllChangesFromVersions(id: String, allVersions: IndexedSeq[LocalDate]) = {
    val idSpecificExporter = new IDSpecificChangeExporter(id,allVersions)
    idSpecificExporter.exportAllChanges()
    //we probably need to track all change records and their last values

    //TODO: continue intervals until end (?)
...
//    }
  ...
  private def addNewChanges(finalChangeCube: ChangeCube,
    ...
    newCube.allChanges.foreach { case Change(t, e, p, v) => {
      if (enteredInitialValues.getOrElse((e, p), false) == false && t != GLOBAL_CONFIG.STANDARD_TIME_FRAME_START) {
  ...
  def addInitialHistoryToChangeCube(e: Long, p: Int, columnSetsAtTimestamp: mutable.TreeMap[LocalDate, Set[Int]], colSetsToConsider: mutable.TreeMap[LocalDate, Set[Int]], finalChangeCube: ChangeCube) = {
    ...
    if (columnSetsAtTimestamp.firstKey != GLOBAL_CONFIG.STANDARD_TIME_FRAME_START) {
      //add non-existant dataset until the first timestamp
      finalChangeCube.addChange(Change(GLOBAL_CONFIG.STANDARD_TIME_FRAME_START, e, p, ReservedChangeValues.NOT_EXISTANT_DATASET))
    } else {
      lastVal = null
    }
    colSetsToConsider.foreach { case (ts, colSet) => {
      if (colSet.isEmpty) {
        //dataset delete!
        if(lastVal == ReservedChangeValues.NOT_EXISTANT_DATASET){
          println()
        }
  ...
  def exportAllChanges(id: String) = {
    ...
    logger.debug(s"Exporting changes in versions: $allVersions")
