...
import de.hpi.socrata.{DatasetInstance, JsonReadable, JsonWritable}
...
object DatasetVersionHistory extends JsonReadable[DatasetVersionHistory] with StrictLogging{
  def load() = fromJsonObjectPerLineFile(Socrata_IOService.getCleanedVersionHistoryFile().getAbsolutePath)
  ...
  def removeVersionIgnore(histories: Iterable[DatasetVersionHistory]) = {
    val versionIgnoreFile = Socrata_IOService.getVersionIgnoreFile()
    val versionIgnores = Source.fromFile(versionIgnoreFile)
      ...
      .map((s:String) => {
        val tokens = s.split(",")
        DatasetInstance(tokens(0),LocalDate.parse(tokens(1),Socrata_IOService.dateTimeFormatter))
      })
      .groupBy(_.id)
    histories.foreach(h => {
