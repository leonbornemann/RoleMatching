...
import de.hpi.socrata.history.DatasetVersionHistory
import de.hpi.socrata.metadata.DatasetMetadata
import de.hpi.socrata.parser.JsonDataParser
import de.hpi.socrata.simplified.RelationalDataset
import de.hpi.socrata.tfmp_input.association.AssociationIdentifier
import de.hpi.socrata.{DatasetInstance, OldLoadedRelationalDataset}
...
object Socrata_IOService extends StrictLogging {
  def STANDARD_TIME_FRAME_START: LocalDate = GLOBAL_CONFIG.STANDARD_TIME_FRAME_START

  def STANDARD_TIME_FRAME_END = GLOBAL_CONFIG.STANDARD_TIME_FRAME_END

  def dateTimeFormatter: DateTimeFormatter = GLOBAL_CONFIG.dateTimeFormatter
  ...
  def getTemporalTableBinaryFile(id: String, dttID: Option[AssociationIdentifier]) = {
    if (!dttID.isDefined)
  ...
  def getTemporalColumnFile(id: String, attrId: Int): File = {
    Socrata_Synthesis_IOService.createParentDirs(new File(s"TEMPORAL_COLUMN_DIR/$id/${id}_$attrId.json"))
  }
  ...
  def getAllSimplifiedDataVersionsForTimeRange(id: String, begin: LocalDate, end: LocalDate) = {
    val files = (begin.toEpochDay to end.toEpochDay)
      ...
      .map(d => (d, new File(getSimplifiedDatasetFile(hpi.socrata.DatasetInstance(id, d)))))
      .filter(t => t._2.exists())
      .toMap
    files
  }
  ...
  def readCleanedDatasetLineages() = DatasetVersionHistory.fromJsonObjectPerLineFile(Socrata_IOService.getCleanedVersionHistoryFile().getAbsolutePath)
  ...
  def extractMinimalHistoryInRange(startVersion: LocalDate, endVersion: LocalDate) = {
    ...
    if (startVersion == Socrata_IOService.getSortedDatalakeVersions().head) {
  ...
  def getSimplifiedDataDir(data: LocalDate) = createAndReturn(new File(SIMPLIFIED_UNCOMPRESSED_DATA_DIR + s"/${GLOBAL_CONFIG.dateTimeFormatter.format(data)}"))
