...
import de.hpi.socrata.OldLoadedRelationalDataset
import de.hpi.socrata.io.Socrata_IOService
import de.hpi.socrata.parser.exceptions.{ContainsArrayException, FirstElementNotObjectException, SchemaMismatchException}
...
class JsonDataParser extends StrictLogging{
  ...
  def parseAllJson(dir:File,version:LocalDate) = {
    ...
    files.foreach(f => {
      ...
      try{
        val ds = parseJsonFile(f,Socrata_IOService.filenameToID(f),version)
