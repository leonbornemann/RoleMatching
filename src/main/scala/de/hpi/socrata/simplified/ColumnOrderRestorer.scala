...
class ColumnOrderRestorer extends StrictLogging{
  ...
  def restoreAllInVersion(version:LocalDate) = {
    Socrata_IOService.cacheMetadata(version)
    val dir = Socrata_IOService.getSimplifiedDataDir(version)
    var count =0
    dir.listFiles.foreach(f => {
      count += 1
      restoreInDataset(Socrata_IOService.filenameToID(f),version)
      if (count % 1000 == 0) {
  ...
  def restoreForAllForIdList(ids:Set[String]) = {
    ...
    versionToIds.foreach{case (v,ids) => {
      ...
      Socrata_IOService.cachedMetadata.clear()
    }}
  }

  def getBaseNameToPosition(id: String): collection.Map[String, Int] = {
    val csvFile = Socrata_IOService.getCSVFile(id)
    if(!csvFile.exists())
      Map()
    else {
  ...
  def correctInDataset(ds:RelationalDataset) = {
    val dsOld = Socrata_IOService.tryLoadDataset(DatasetInstance(ds.id,ds.version),true)
  ...
  def restoreInDataset(id:String, version:LocalDate,runCorrection:Boolean = false) = {
    Socrata_IOService.cacheMetadata(version)
    val mdForDs = Socrata_IOService.cachedMetadata(version).get(id)
    var ds: RelationalDataset = null
    try {
      ds = Socrata_IOService.loadSimplifiedRelationalDataset(hpi.socrata.DatasetInstance(id, version))
    } catch {
      case e: Throwable => {
    ...
    if(ds!=null) {
      ...
      ds.toJsonFile(new File(Socrata_IOService.getSimplifiedDatasetFile(hpi.socrata.DatasetInstance(id, version)) + "_repaired.json?"))
    }
  }
