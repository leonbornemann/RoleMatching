...
import de.hpi.socrata.tfmp_input.table.nonSketch.FactLineage.WILDCARD_VALUES
import de.hpi.socrata.tfmp_input.table.{AbstractTemporalField, TemporalFieldTrait}
...
case class FactLineage(lineage:mutable.TreeMap[LocalDate,Any] = mutable.TreeMap[LocalDate,Any]()) extends AbstractTemporalField[Any] with Serializable{
  ...
  def keepOnlyStandardTimeRange = FactLineage(lineage.filter(!_._1.isAfter(Socrata_IOService.STANDARD_TIME_FRAME_END)))
