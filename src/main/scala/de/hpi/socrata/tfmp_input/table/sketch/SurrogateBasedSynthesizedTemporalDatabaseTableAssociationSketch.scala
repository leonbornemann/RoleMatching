...
import de.hpi.socrata.io.Socrata_Synthesis_IOService.{OPTIMIZATION_INPUT_ASSOCIATION_SKETCH_DIR, OPTIMIZATION_INPUT_FULL_TIME_RANGE_ASSOCIATION_SKETCH_DIR, createParentDirs}
import de.hpi.socrata.tfmp_input.association.{AssociationIdentifier, AssociationSchema}
import de.hpi.socrata.tfmp_input.table.sketch.SurrogateBasedSynthesizedTemporalDatabaseTableAssociationSketch.{getFullTimeRangeFile, getOptimizationInputAssociationSketchFile}
import de.hpi.socrata.tfmp_input.table.{AbstractSurrogateBasedTemporalRow, AbstractSurrogateBasedTemporalTable, TemporalDatabaseTableTrait, TemporalFieldTrait}
import de.hpi.socrata.tfmp_input.{BinaryReadable, SynthesizedDatabaseTableRegistry}
...
object SurrogateBasedSynthesizedTemporalDatabaseTableAssociationSketch extends BinaryReadable[SurrogateBasedSynthesizedTemporalDatabaseTableAssociationSketch]{
  ...
  def getOptimizationInputAssociationSketchFile(id: AssociationIdentifier) = {
    Socrata_Synthesis_IOService.createParentDirs(new File(s"${OPTIMIZATION_INPUT_ASSOCIATION_SKETCH_DIR(id.subdomain)}/${id.viewID}/${id.compositeID}.binary"))
  }

  def getOptimizationInputAssociationSketchParentDirs(subdomain:String) = {
    Socrata_Synthesis_IOService.createParentDirs(new File(s"${OPTIMIZATION_INPUT_ASSOCIATION_SKETCH_DIR(subdomain)}/")).listFiles()
  }
