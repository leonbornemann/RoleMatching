...
class TableTupleFindIndex[A](sketchA: TemporalDatabaseTableTrait[A], sketchB: TemporalDatabaseTableTrait[A]) extends IterableTupleIndex[A]{
  ...
  class TupleGroupIterator(skipWildCardBuckets: Boolean) extends Iterator[TupleGroup[A]] {
    ...
    override def next(): TupleGroup[A] = {
      ...
      val fromTableA = indexA.getOrElse(key,IndexedSeq()).map(rID => fact.TupleReference(sketchA,rID))
      val wildcardsA = wildcardKeys.toIndexedSeq
        .map(k => indexA.getOrElse(k,IndexedSeq()).map(rID => fact.TupleReference(sketchA,rID)))
        .flatten
      val fromTableB = indexB.getOrElse(key,IndexedSeq()).map(rID => fact.TupleReference(sketchB,rID))
      val wildcardsB = wildcardKeys.toIndexedSeq
        .map(k => indexB.getOrElse(k,IndexedSeq()).map(rID => fact.TupleReference(sketchB,rID)))
        .flatten
