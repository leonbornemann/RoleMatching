...
import de.hpi.tfm.data.socrata.simplified.{Attribute, RelationalDataset}
...
class IDSpecificChangeExporter(id: String, var allVersions: IndexedSeq[LocalDate]) {
    ...
    ds.rows.foreach(r => {
      ...
      (0 until r.fields.size).foreach(attrIndex => {
        ...
        if (fieldToLineageMap.contains(field)) {
          ...
          assert(curVersion == IOService.STANDARD_TIME_FRAME_START && fieldToLineageMap(field).lineage.isEmpty || fieldToLineageMap(field).lineage.lastKey.isBefore(curVersion))
  ...
  private def updateAttributeExistence(curTimeInterval: TimeInterval, ds: RelationalDataset) = {
    ...
    firstTimeAttrInserts.foreach(id => {
      ...
      if(curTimeInterval.begin!=IOService.STANDARD_TIME_FRAME_START){
        val prevInterval = TimeInterval(IOService.STANDARD_TIME_FRAME_START, Some(curTimeInterval.begin.minusDays(1)))
  ...
  def exportAllChanges() = {
    ...
    if(firstVersion.isAfter(IOService.STANDARD_TIME_FRAME_START)){
      allVersions = IndexedSeq(IOService.STANDARD_TIME_FRAME_START) ++ allVersions
    }
    ...
    finalChangeCube.toJsonFile(new File(IOService.getChangeFile(id)))
  ...
  private def runtIntegrityCheck = {
    fieldToLineageMap.foreach { case ((e, p), fieldLineage) => {
      ...
      assert(fieldLineage.lineage.firstKey == IOService.STANDARD_TIME_FRAME_START)
  ...
  private def versionExists(curVersion: LocalDate) = {
    new File(IOService.getSimplifiedDatasetFile(socrata.DatasetInstance(id, curVersion))).exists()
  }
}
