...
object DBSynthesis_IOService extends StrictLogging{
  ...
  def COMPATIBILITY_GRAPH_DIR = OPTIMIZATION_INPUT_DIR + "/compatibilityGraphs/"
  ...
  def getSurrogateBasedDecomposedTemporalTableFile(id: AssociationIdentifier) = {
  ...
  def getDecomposedTemporalTableSketchFile(tableID: AssociationIdentifier, getVariantName: String) = {
    ???
