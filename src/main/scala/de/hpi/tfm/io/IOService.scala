...
import de.hpi.tfm.data.socrata.metadata.DatasetMetadata
import de.hpi.tfm.data.socrata.parser.JsonDataParser
import de.hpi.tfm.data.socrata.simplified.RelationalDataset
import de.hpi.tfm.data.socrata.{DatasetInstance, OldLoadedRelationalDataset}
...
object IOService extends StrictLogging{
  ...
  def getAllSimplifiedDataVersions(id: String) = {
    val files = getStandardTimeRange
      .map(d => (d,new File(getSimplifiedDatasetFile(socrata.DatasetInstance(id,d)))))
  ...
  var STANDARD_TIME_FRAME_START = LocalDate.parse("2019-11-01", IOService.dateTimeFormatter)
